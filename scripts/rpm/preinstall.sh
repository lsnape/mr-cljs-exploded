...
then
  /bin/echo "Mrcljs is not running"
else
  sleepCounter=0
  sleepIncrement=2
  waitTimeOut=600
  ...
  /bin/echo "Mrcljs is running, stopping service"
...
then
  mkdir -p $prefixDir
  /usr/sbin/useradd -r -s /sbin/nologin -d $prefixDir -m -c "Mrcljs user for the Mrcljs service" $APP_NAME 2> /dev/null || :
fi
