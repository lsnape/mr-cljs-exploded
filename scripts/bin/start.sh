...
if [ -n "$PIDS" ]
then
  echo "Mrcljs is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/mrcljs.out
ERR_FILE=${LOGGING_PATH}/mrcljs.err
...
echo "Giving Mrcljs $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Mrcljs didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Mrcljs failed"
else
  echo "Starting Mrcljs succeeded"
fi
